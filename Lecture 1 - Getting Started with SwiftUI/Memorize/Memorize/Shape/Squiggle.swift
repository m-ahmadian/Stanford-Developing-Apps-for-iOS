...
struct Squiggle: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        path.addCurve(
            to: CGPoint(x: rect.maxX - 2 * offset, y: rect.maxY - 3 * offset),
            control1: CGPoint(x: rect.maxX + 8 * offset, y: rect.minY + 4 * offset),
            control2: CGPoint(x: rect.minX, y: rect.midY + 2 * offset)
        )
        path.addQuadCurve(
            to: CGPoint(x: rect.maxX - 3 * offset, y: rect.maxY - 1.25 * offset),
            control: CGPoint(x: rect.maxX - 0.5 * offset, y: rect.maxY - 1.25 * offset)
        )
        path.addCurve(
            to: CGPoint(x: rect.minX + 2 * offset, y: rect.minY + 3 * offset),
            control1: CGPoint(x: rect.minX - 8 * offset, y: rect.maxY - 4 * offset),
            control2: CGPoint(x: rect.maxY, y: rect.midY - 2 * offset)
        )
        path.addQuadCurve(
            to: CGPoint(x: rect.minX + 3 * offset, y: rect.minY + 1.25 * offset),
